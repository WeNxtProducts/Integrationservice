...
@Service
public class YiChargesServiceImpl implements YiChargesService {
	...
	@Autowired
	private YI_Charge_DETAILRepo chargerrepo;

	@Override

	public String saveAll(String payload) {
		...
		try {
			...
			Field[] fields = YiChargeDetail.class.getDeclaredFields();

			List<YiChargeDetail> entities = new ArrayList<>();
			for (int i = 0; i < entitiesArray.length(); i++) {
				JSONObject entityJson = entitiesArray.getJSONObject(i);
				YiChargeDetail entity = new YiChargeDetail();
			...
			// Save all entities
			List<YiChargeDetail> savedEntities = chargerrepo.saveAll(entities);
			int count = savedEntities.size();

			response.put(statusCode, successCode);
			response.put(messageCode, "Pay To charges Inserted Successfully");
